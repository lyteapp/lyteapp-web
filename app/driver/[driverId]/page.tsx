...
export default async function DriverPage(
  ...
  const { data: activeDelivery } = await supa
    .from('deliveries')
    .select('id, customer_name, customer_phone, delivery_address, notes, status, picked_up_at, order_id, customer_lat, customer_lng, orders(total, payment_method, payment_proof_url), zone:zone_id(name, fee, color)')
  ...
  type OrderJoin = { total: number; payment_method: string; payment_proof_url: string | null } | null
  type ZoneJoin  = { name: string; fee: number; color: string } | null

  const flatDelivery = activeDelivery ? {
    ...activeDelivery,
    order_total:          (activeDelivery.orders as unknown as OrderJoin)?.total ?? null,
    order_payment_method: (activeDelivery.orders as unknown as OrderJoin)?.payment_method ?? null,
    order_proof_url:      (activeDelivery.orders as unknown as OrderJoin)?.payment_proof_url ?? null,
    zone_name:  (activeDelivery.zone as unknown as ZoneJoin)?.name  ?? null,
    zone_fee:   (activeDelivery.zone as unknown as ZoneJoin)?.fee   ?? null,
    zone_color: (activeDelivery.zone as unknown as ZoneJoin)?.color ?? null,
  } : null
